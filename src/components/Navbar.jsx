  ...
  const NavDropdown = ({ label, items }) => (
    <div className="relative group h-full flex items-center">
      <button className="px-3 py-2 rounded-md text-sm font-medium text-slate-700 hover:bg-slate-50 flex items-center gap-1 focus:outline-none">
        {label}
        <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 opacity-50 group-hover:rotate-180 transition-transform duration-200" viewBox="0 0 20 20" fill="currentColor">
        ...
        <ul className="py-2">
          {items.map((item) => (
            <li key={item.to}>
              <NavLink
                to={item.to}
                className="block px-4 py-2 text-sm text-slate-700 hover:bg-slate-50 hover:text-cuGreen transition-colors"
    ...
    <header className="sticky top-0 z-50 bg-white/80 backdrop-blur-md border-b border-slate-200">
      <div className="container flex items-center justify-between h-16">
        ...
        <nav className="hidden md:flex items-center gap-1">
          <NavLink to="/" className={({ isActive }) => `px-3 py-2 rounded-md text-sm font-medium ${isActive ? "text-cuGreen bg-slate-100" : "text-slate-700 hover:bg-slate-50"}`}>Home</NavLink>
          
          <NavLink to="/about" className={({ isActive }) => `px-3 py-2 rounded-md text-sm font-medium ${isActive ? "text-cuGreen bg-slate-100" : "text-slate-700 hover:bg-slate-50"}`}>About</NavLink>
          ...
          <Link to="/booking" className="btn-primary ml-2">Book a Slot</Link>
        </nav>
        <button
      ...
      {open && (
        <div className="md:hidden border-t border-slate-200 bg-white max-h-[80vh] overflow-y-auto">
          <div className="container py-3 space-y-4">
            <NavLink to="/" className="block px-3 py-2 rounded-md text-sm font-medium text-slate-700 hover:bg-slate-50" onClick={() => setOpen(false)}>Home</NavLink>
            ...
            {[
              ...
              <div key={section.label}>
                <div className="px-3 py-1 text-xs font-bold text-slate-400 uppercase tracking-wider">{section.label}</div>
            ...
            <NavLink to="/about" className="block px-3 py-2 rounded-md text-sm font-medium text-slate-700 hover:bg-slate-50" onClick={() => setOpen(false)}>About</NavLink>
            <Link to="/booking" className="btn-primary w-full text-center mt-4">Book a Job</Link>
          </div>
        </div>
      )}
    </header>
